...
function runSelfUpdate() {
  return new Promise((resolve, reject) => {
    ...
    if (process.platform === 'win32') {
      // npm/global CLI shims are .cmd files on Windows. Spawning a .cmd file
      // directly with shell:false throws EINVAL on Node/Windows. Invoke it
      // through cmd.exe explicitly instead. UPDATE_REPO is a fixed constant.
      cmd = process.env.ComSpec || 'cmd.exe';
      args = ['/d', '/s', '/c', `dsh plugin --profile web add "${UPDATE_REPO}"`];
    } else {
      cmd = 'dsh';
      args = ['plugin', '--profile', 'web', 'add', UPDATE_REPO];
    }
